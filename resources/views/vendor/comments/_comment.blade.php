...
<div id="comment-{{ $comment->getKey() }}" class="media">
    <img class="img-fluid" style="height:60px; !important"src="{{asset('avatars/'.$comment->commenter->avatar)}}" alt="{{ $comment->commenter->name }} Avatar">
    <div class="media-body">
        <h5 class="mt-0 mb-1 ml-2">{{ $comment->commenter->fname }} {{ $comment->commenter->lname }}<small class="text-muted">- {{ $comment->created_at->diffForHumans() }}</small></h5>
        <div class="ml-2"style="white-space: pre-wrap;">{!! $markdown->line($comment->comment) !!}</div>
